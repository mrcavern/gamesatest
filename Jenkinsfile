            ...
            steps {
                ...
                withCredentials([usernamePassword(credentialsId: 'webserver_login', usernameVariable: 'USERNAME', passwordVariable: 'SSHPASS')]) {
                    script {
                        ...
                        sh "sshpass -p 'Cueva\$\$18' ssh -o StrictHostKeyChecking=no -p2299 $USERNAME@$prod_ip \"docker run --restart always --name train-schedule -p 8080:8080 -d dcuevas86/gamesademo:${env.BUILD_NUMBER}\""
                    }
                }
            }
        }
